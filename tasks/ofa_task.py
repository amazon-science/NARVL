...
class OFAConfig(FairseqDataclass):
    ...
    add_blank_id_to_dict: Optional[bool] = field(
        default=False,
        metadata={"help": "add blank id for NAT models"}
    )
...
class OFATask(FairseqTask):
    ...
    def setup_task(cls, cfg: DictConfig, **kwargs):
        ...
        if cfg.add_blank_id_to_dict:
            blank_sym = '<<BLANK>>'
            src_dict.axe_eps_idx = src_dict.add_symbol(blank_sym, ignore_in_string=True)
            tgt_dict.axe_eps_idx = tgt_dict.add_symbol(blank_sym, ignore_in_string=True)
