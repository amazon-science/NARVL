...
class DetectionDataset(OFADataset):
    ...
    def process_detection(self, index):
        ...
        _, indices_x = torch.sort(boxes_target["boxes"][:, 0], descending=True)
        _, indices_y = torch.sort(boxes_target["boxes"][indices_x][:, 1], descending=True)
        ...
        # if '<bin_0> <bin_0> <bin_500> <bin_500>' in ' '.join(quant_boxes):
        #     from torchvision.utils import save_image
        #     save_image(patch_image, f'img{image_id}.png')
        #     print('gt', quant_boxes)
        #     image.save('test.png')
        #     print(image_id)
            # self.bpe.decode(' 1097')
            # breakpoint()
